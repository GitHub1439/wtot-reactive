		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input id="fileImage" type="file" size="30" accept="'+para.file_accept+'" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					...
					html += '	</div>';
					html += '	<input name="dosubmit" type="hidden" value="dosubmit" />';					  
					html += '</form>';
				}else{
				    ...
				    html += '							<img class="upload_image" src="statics/js/zyFile/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				if(files.length>para.file__number){
					alert("请选择"+para.file__number+"个以内的文件");
					return arrFiles;	
				}else{
					for (var i = 0, file; file = files[i]; i++) {
						console.info(file);
						if (number < para.file__number) {
							if (file.size > para.file__size) {
								alert('您这个"'+ file.name +'"文件大小过大');	
							}else{
								if (IsType(file.name)==false) {
									alert('您这个"'+ file.name +'"格式不正确');	
								}else{
									number++;
									// 在这里需要判断当前所有文件中
									arrFiles.push(file);	
							...
							var othernum = para.file__number-number;
							alert("还可以选择"+othernum+"个文件");
						}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "statics/js/zyFile/control/images/fileType/";
				var index1=file.name.lastIndexOf(".");
				var index2=file.name.length;
				var suffix=file.name.substring(index1,index2);//后缀名
				if(suffix.indexOf("rar") > 0){
					fileImgSrc = fileImgSrc + "rar.png";
				}else if(suffix.indexOf("zip") > 0){
					fileImgSrc = fileImgSrc + "zip.png";
				}else if(suffix.indexOf("text") > 0){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="module_'+file.index+'" tailor="'+para.module+'"></p>';
					html += '	<p id="catid_'+file.index+'" tailor="'+para.catid+'"></p>';
					html += '	<p id="userid_'+file.index+'" tailor="'+para.userid+'"></p>';
					html += '</div>';
			...
			this.createCorePlug = function(){
				var params = {
							...
							if($(".file_edit").length>0){
								if($("head").html().indexOf("zyPopup")<0){  // 代表没有加载过js和css文件
									...
									$("<link>").attr({ rel: "stylesheet",
								        type: "text/css",
								        href: "statics/js/zyFile/zyPopup/css/zyPopup.css"
								    }).appendTo("head");
									$.getScript("statics/js/zyFile/zyPopup/js/zyPopup.js", function(){
										// 编辑方法
					...
					onDelete: function(file, files) {
						...
						$("#imgbox #jilu_"+ file.index).remove();

						var length = $("#imgbox").children("img").length;
						var strs = filenames = '';
						for(var i=0;i<length;i++){
							strs += '|'+$("#imgbox").children("img").eq(i).attr('path');
							filenames += '|'+$("#imgbox").children("img").eq(i).attr('title');
						}
						$("#att-status").html();
						$('#att-name').html();
						$('#att-status').html(trim(strs));
						$('#att-name').html(trim(filenames));
					...
					onSuccess: function(file, response) {
						...
						if (response) {
							var img = '<img id="jilu_'+file.index+'" path="'+response+'" title="'+file.name+'"/>';
							$('#imgbox').append(img);
							$('#att-status').append('|'+response);
							$('#att-name').append('|'+file.name);
						}
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
