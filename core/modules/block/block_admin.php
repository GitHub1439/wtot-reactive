...
class block_admin extends admin {
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$pos = isset($_POST['pos']) && trim($_POST['pos']) ? trim($_POST['pos']) : showmessage(L('illegal_operation'), HTTP_REFERER);
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->update(array('name'=>$name,'type'=>$type, 'siteid'=>$this->siteid), array('id'=>$id))) {
				//设置权限
				$priv = isset($_POST['priv']) ? $_POST['priv'] : '';
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			} elseif ($data['type'] == 1 || $data['type'] == 3) {
			...
			if ($this->db->update($sql, array('id'=>$id))) {
				...
				//showmessage(L('operation_success'), '?m=block&c=block_admin&a=init');
