...
class index {
    ...
    public function init() {
        ...
        if(isset($_GET['q'])) {
            ...
            if ($_GET['catid']) {
               $i = 0; 
               $arrcatid =  explode("_", $_GET['catid']); 
               foreach ($arrcatid as $value) {
                if ($i==0) {
                     $catid = diy_child($value); 
                }else{
                     $catid .= ",".diy_child($value);    
                }
               $i++;  
            ...
            if($page==1 && !$setting['sphinxenable']) {
                //精确搜索
                $commend = $this->db->get_one("`siteid`= '$siteid' $sql_tid $sql_time $catid AND `data` like '%$q%'");
            } else {
                $commend = '';
            }
            //如果开启sphinx
            if($setting['sphinxenable']) {
                ...
                $sql = "`siteid`= '$siteid' $sql_tid $sql_time $catid AND `data` like '%$q%'";
