		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				if(isset($_POST['dosubmit'])) {
					...
					showmessage(L('add_success'),'?m=content&c=content&a=init&menuid=822&catid='.$catid.'&pc_hash='.$_SESSION[pc_hash]);
				...
				$_POST['info'] = trim_script($_POST['info']);
				if($_POST['edit']) {
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				if(isset($_POST['dosubmit'])) {
					...
					showmessage(L('update_success'),'?m=content&c=content&a=init&menuid=822&catid='.$catid.'&pc_hash='.$_SESSION[pc_hash]);
				} else {
	...
	public function remove() {
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['fromtype']==0) {
				...
				if($modelid!=$_POST['f_modelid']) showmessage(L('来源模型与目标模型不一致！'),HTTP_REFERER);
