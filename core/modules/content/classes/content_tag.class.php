...
class content_tag {
	...
	public function count($data) {
		if($data['action'] == 'lists') {
			...
			$tiaojian .= " and posids = '".$data['posids']."'";
			}
			//
			if(isset($data['noid'])) {
			$tiaojian .= " and id not in (".$data['noid'].") ";
			}
			...
			if(isset($data['where'])) {
				...
				if($this->category[$catid]['child']) {
					...
					$sql = "status=99 ".$tiaojian." AND catid IN ($catids_str)".$thumb;
				} else {
					$sql = "status=99 ".$tiaojian." AND catid='$catid'".$thumb;
				}
			}
	...
	public function lists($data) {
		...
		if (strpos($data['order'],'inputtime') == false) {
			$order = $data['order'].",inputtime DESC";
		}else{
			$order = $data['order'];	
		}
	 ...
	 * @param $data
	 */
	public function wapcategory($data) {
		$data['catid'] = intval($data['catid']);
		$array = array();
		$siteid = $data['siteid'] && intval($data['siteid']) ? intval($data['siteid']) : get_siteid();
		$categorys = getcache('category_content_'.$siteid,'commons');
		...
		foreach ($categorys as $catid=>$cat) {
			if($i>$data['limit']) break;
			if((!$cat['iswapmenu']) || $siteid && $cat['siteid']!=$siteid) continue;
			if (strpos($cat['url'], '://') === false) {
				$cat['url'] = substr($site['domain'],0,-1).$cat['url'];
			}
			if($cat['parentid']==$data['catid']) {
