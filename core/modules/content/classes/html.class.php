...
class html {
    private $siteid, $url, $html_root, $wap_html_root, $queue, $categorys;
    public function __construct() {
        ...
        $this->wap_html_root = pc_base::load_config('system', 'wap_html_root');
    ...
    public function show($file, $data = '', $array_merge = 1, $action = 'add', $upgrade = 0) {
            ...
            if ($CONTENT_POS !== false) {
                ...
                foreach ($pageurls as $page => $urls) {
                    ...
                    if ($this->wap['status'] == 1) {
                        $GLOBALS['static_ly'] = 'wap';
			        	if ($this->html_root=="") {
			        		$wap_pagefile = $this->wap_html_root . $pagefile;
			        	}else{
			        		$wap_pagefile = str_replace($this->html_root,$this->wap_html_root,$pagefile);
			        	}
			        	$wap_pagefile = PHPCMS_PATH . $wap_pagefile;
			        	$wap_template = $CAT['setting']['wap_show_template'] ? $CAT['setting']['wap_show_template'] : 'show';
			            include template('wap', $wap_template);
			            $this->createhtml($wap_pagefile);	
			        }
                    $GLOBALS['static_ly'] = 'pc';
        ...
        if ($this->wap['status'] == 1) {
            $GLOBALS['static_ly'] = 'wap';
        	if ($this->html_root=="") {
        		$wap_file = $this->wap_html_root . $file;
        	}else{
        		$wap_file = str_replace($this->html_root,$this->wap_html_root,$file);
        	}
        	$wap_file = substr(PHPCMS_PATH, 0, -1) . $wap_file;
            $wap_template = $CAT['setting']['wap_show_template'] ? $CAT['setting']['wap_show_template'] : 'show';
            include template('wap', $wap_template);
            $this->createhtml($wap_file);	
        }
        $GLOBALS['static_ly'] = 'pc';
     ...
     * @param  $upgrade 是否是升级数据
     */
/*    public function wap_show($file, $data = '', $array_merge = 1, $action = 'add', $upgrade = 0) {
        if ($this->wap['status'] == 1) {
            if ($upgrade) $file = '/' . ltrim($file, WEB_PATH);
            $allow_visitor = 1;
            $id = $data['id'];
            ...
            //添加到发布点队列
            //当站点为非系统站点
            $file = str_replace($this->html_root, '', $file);
            if ($this->siteid != 1) {
                $site_dir = $this->sitelist[$this->siteid]['dirname'];
                $file = $this->wap_html_root . '/' . $site_dir . $file;
            } else {
                $file = $this->wap_html_root . $file;
            }
            $this->queue->add_queue($action, $file, $this->siteid);
            $modelid = $CAT['modelid'];
            require_once CACHE_MODEL_PATH . 'content_output.class.php';
            ...
            $SEO = seo($siteid, $catid, $title, $description, $seo_keywords);
            $ishtml = 1;
            $template = $template ? $template : $CAT['setting']['wap_show_template'];
            //分页处理
            $pages = $titles = '';
            if ($paginationtype == 1) {
                //自动分页
            ...
            if ($paginationtype != 0) {
                ...
                if ($CONTENT_POS !== false) {
                    ...
                    foreach ($pageurls as $page => $urls) {
                        ...
                        $pagefile = $urls[1];
                        if ($this->siteid != 1) {
                            $pagefile = $this->wap_html_root . '/' . $site_dir . $pagefile;
                        }
                        $this->queue->add_queue($action, $pagefile, $this->siteid);
                        $pagefile = PHPCMS_PATH . $pagefile;
                        ob_start();
    ...
    public function category($catid, $page = 0) {
        ...
        if ($this->wap['status'] == 1) {
        	...
        	$wap_file = str_replace(substr(PHPCMS_PATH, 0, -1),"",$file);
        	if ($this->html_root=="") {
        		$wap_file = $this->wap_html_root . $wap_file;
        	}else{
        		$wap_file = str_replace($this->html_root,$this->wap_html_root,$wap_file);
        	}
        	$wap_file = substr(PHPCMS_PATH, 0, -1) . $wap_file;
            include template('wap', $wap_template);
            $this->createhtml($wap_file, $copyjs);
        }
        $GLOBALS['static_ly'] = 'pc';
        include template('content', $template);
     ...
     * @param $page 当前页数
     */
/*    public function wap_category($catid, $page = 0) {
        if ($this->wap['status'] == 1) {
            $CAT = $this->categorys[$catid];
            @extract($CAT);
            if (!$ishtml) return false;
            ...
            if ($this->siteid != 1) {
                ...
                if ($create_to_html_root) {
                    ...
                    $base_file = '/' . $site_dir . $this->wap_html_root . $base_file;
                }
            }
            //判断二级域名是否直接绑定到该栏目
            $root_domain = preg_match('/^((http|https):\/\/)([a-z0-9\-\.]+)\/$/', $CAT['url']) ? 1 : 0;
            ...
            if ($create_to_html_root) {
                if ($this->siteid == 1) {
                    ...
                    $file = PHPCMS_PATH . substr($this->wap_html_root, 1) . $base_file;
                }
                //添加到发布点队列
                $this->queue->add_queue('add', $base_file, $this->siteid);
                //评论跨站调用所需的JS文件
                ...
                $file = PHPCMS_PATH . substr($this->wap_html_root, 1) . $base_file;
                //添加到发布点队列
                $this->queue->add_queue('add', $this->wap_html_root . $base_file, $this->siteid);
                //评论跨站调用所需的JS文件
                if (substr($base_file, -10) == 'index.html' && $count_number == 3) {
                    $copyjs = 1;
                    $this->queue->add_queue('add', $this->wap_html_root . $base_file, $this->siteid);
            ...
            if ($type == 0) {
                $template = $setting['wap_category_template'] ? $setting['wap_category_template'] : 'category';
                $template_list = $setting['wap_list_template'] ? $setting['wap_list_template'] : 'list';
                $template = $child ? $template : $template_list;
                $arrparentid = explode(',', $arrparentid);
                $top_parentid = $arrparentid[1] ? $arrparentid[1] : $catid;
                $array_child = array();
                ...
                $datas = $this->page($catid);
                if ($datas) extract($datas);
                $template = $setting['wap_page_template'] ? $setting['wap_page_template'] : 'page';
                $parentid = $CATEGORYS[$catid]['parentid'];
                $arrchild_arr = $CATEGORYS[$parentid]['arrchildid'];
                if ($arrchild_arr == '') $arrchild_arr = $CATEGORYS[$catid]['arrchildid'];
                $arrchild_arr = explode(',', $arrchild_arr);
    ...
    public function index() {
        ...
        if ($this->wap['status'] == 1) {
            if ($this->siteid == 1) {
                $wap_file = PHPCMS_PATH .$this->wap_html_root. '/index.html';
                //添加到发布点队列
                $this->queue->add_queue('edit', '/index.html', $this->siteid);
            } else {
                $site_dir = $this->sitelist[$this->siteid]['dirname'];
                $wap_file = $this->wap_html_root . '/' . $site_dir . '/index.html';
                //添加到发布点队列
                $this->queue->add_queue('edit', $wap_file, $this->siteid);
                $wap_file = PHPCMS_PATH . $wap_file;
            }
            include template('wap', 'index', $style);
            $this->createhtml($wap_file, 1);    
        }
...
/*    public function wap_index() {
        if ($this->siteid == 1) {
            $file = $this->wap_html_root . '/index.html';
            //添加到发布点队列
            $this->queue->add_queue('edit', $file, $this->siteid);
            $file = PHPCMS_PATH . $file;
        } else {
            $site_dir = $this->sitelist[$this->siteid]['dirname'];
            $file = $this->wap_html_root . '/' . $site_dir . '/index.html';
            //添加到发布点队列
            $this->queue->add_queue('edit', $file, $this->siteid);
            $file = PHPCMS_PATH . $file;
        }
    ...
    public function page($catid) {
        ...
        $data = $this->page_db->get_one(array(
            'catid' => $catid
        ));
        return $data;
    }
