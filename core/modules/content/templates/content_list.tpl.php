...
<a href="?m=content&c=content&a=init&steps=100&catid=<?php echo $catid;?>&pc_hash=<?php echo $pc_hash;?>" <?php if($steps==100) echo 'class=on';?>><em><?php echo '回收站';?></em></a><span>|</span>
...
<tbody>
	...
	?>
		...
		<td align='center'><a href="?m=content&c=content&a=edit&catid=<?php echo $catid;?>&id=<?php echo $r['id']?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>" ><?php echo L('edit');?></a> | <a href="javascript:view_comment('<?php echo id_encode('content_'.$catid,$r['id'],$this->siteid);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a></td>
	</tr>
     <?php }
	}
	?>
    ...
    <?php
    	 if($steps==100){
    ?>
		<input type="button" class="button" value="<?php echo '还原';?>" onclick="myform.action='?m=content&c=content&a=effective&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return myform.submit();"/>
    <?php
    	}else{
    ?>		
		<input type="button" class="button" value="<?php echo '回收';?>" onclick="myform.action='?m=content&c=content&a=invalid&dosubmit=1&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return confirm_invalid()"/>
    <?php
    	} 
    ?>
...
function confirm_invalid(){
	if(confirm('<?php echo L('确定放入回收站？', array('message' => L('selected')));?>')) $('#myform').submit();
}
