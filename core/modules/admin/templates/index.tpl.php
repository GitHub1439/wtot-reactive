...
<script language="javascript" type="text/javascript" src="<?php echo JS_PATH?>kaifazhe.js"></script>
...
<div class="header">
	<div class="logo lf"><a href="?m=admin&c=index"><img src="<?php echo IMG_PATH.'admin_img/admin_logo.png'?>" /></a></div>
    <div class="rt-col">
    ...
    <div class="col-auto">
    	<div class="log white cut_line"><?php echo L('hello'),$admin_username?>  [<?php echo $rolename?>]<span>|</span><a href="?m=admin&c=index&a=public_logout">[<?php echo L('exit')?>]</a><span>|</span>
    		<a href="<?php echo $currentsite['domain']?>index.php" target="_blank" id="site_homepage"><?php echo L('site_homepage')?></a><span>|</span>
    		<img src="<?php echo IMG_PATH.'icon/lockscreen.png'?>" style="margin-top: -6px;"> <a href="javascript:;" onclick="lock_screen()"><?php echo L('lockscreen')?></a>&nbsp;
...
<div id="content">
	<div class="col-left left_menu">
    	...
    	<div id="Scroll"><div id="leftMain"></div><a href="javascript:;" id="openClose" style="outline-style: none; outline-color: invert; outline-width: medium;" hideFocus="hidefocus" class="open" title="<?php echo L('spread_or_closed')?>"><span class="hidden"><?php echo L('expand')?></span></a></div>
    ...
    <a style="background: #373737; border-radius:3px; color: #fff; font-size:12px;" href="?m=content&c=create_html&a=public_index&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('create_index')?></span></a>
    <a style="background: #e74c3c; border-radius:3px; color: #fff; font-size:12px; margin-right: 15px;" href="?m=admin&c=cache_all&a=init&pc_hash=<?php echo $_SESSION['pc_hash'];?>" target="right"><span><?php echo L('update_backup')?></span></a>
   <!--  <a style="background: #f2c308; border-radius:3px; color: #fff; " href="javascript:art.dialog({id:'map',iframe:'?m=admin&c=index&a=public_map', title:'<?php echo L('background_map')?>', width:'710', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><?php echo runhook('admin_top_left_menu')?> --></div>
    	...
    	<div class="col-1">
        	...
        	<div id="kfz">
				...
				<div class="showMsg" style="text-align:center";>
					<h5>输入开发者密码</h5>
				    <div class="content">
					<input type="password" size="41" id="cat_cookie" class="input-text" value="" >
					<button class="aui_state_highlight" type="submit" onclick="yanzheng()">确定</button>
					</div>
...
$("#openClose").click(function(){
	if($(this).data('clicknum')==1) {
		...
		$("#Scroll #leftMain").show();
	} else {
		$(".left_menu").addClass("left_menu_on");
		$(this).addClass("close");
		$("html").addClass("on");
		 $(this).data('clicknum', 1);
		$("#Scroll #leftMain").hide();
	}
	return false;
});
//写cookies
...
//JS操作cookies方法!
function yanzheng(){	
	     var value=document.getElementById("cat_cookie").value;
	     setCookie('kaifazhe',value,1);
	     $("#rightMain").show();
		 $("#kfz").hide();      
	     _M(7,'?m=admin&c=extend&a=init_extend');
 　　}
...
function _M(menuid,targetUrl) {
	...
	if(menuid!=8) {
		if(menuid ==7) {
            ...
            var eh = 115;
            ch = (wh - eh) + "px";
            document.getElementById( "kfz" ).style.height = ch;
