...
<!--
	...
	$(function(){
		$("#catname").live("keyup keydown change blur",function (){
			var pinyin = $(this).toPinyin();
			pinyin = pinyin.replace(/\s/g, "");
			$("#catdir").val(pinyin.toLowerCase());
		});
...
<li id="tab_setting_1" class="on" onclick="SwapTab('setting','on','',7,1);"><?php echo L('catgory_basic');?></li>

<li id="tab_setting_2" onclick="SwapTab('setting','on','',7,2);"><?php echo L('catgory_createhtml');?></li>

<li id="tab_setting_3" onclick="SwapTab('setting','on','',7,3);"><?php echo L('catgory_template');?></li>

<li id="tab_setting_4" onclick="SwapTab('setting','on','',7,4);"><?php echo L('catgory_seo');?></li>

<li id="tab_setting_5" onclick="SwapTab('setting','on','',7,5);"><?php echo L('catgory_private');?></li>

<li id="tab_setting_6" onclick="SwapTab('setting','on','',7,6);"><?php echo L('catgory_readpoint');?></li>

<li id="tab_setting_7" onclick="SwapTab('setting','on','',7,7);"><?php echo '手机站设置';?></li>
 ...
 <tr>
	...
	<tr id="catdir_tr">
       ...
       <tr>
        ...
        <td><input type="text" name="info[catname_en]" id="catname_en" class="input-text" value=""></td>
	...
	<tr>
		...
		flashupload:false,alowuploadexts:'',allowbrowser:'1',allowuploadnum:'10',authkey:'c8e07e653e467f2f1b2058ee44db799c',
    ...
    <tr>
        ...
        <th><?php echo L('跳转栏目')?>：</th>

        <td>

		<input type="text" name="info[catgory_lianjie]" id="catgory_lianjie" class="input-text" value="0">需要连接的栏目id，默认0连接到第一个子栏目
...
<tr>
  ...
  <th width="200"><?php echo L('available_styles');?>：</th>
      ...
      <tr>
        ...
        <th width="200"><?php echo '是否启用栏目首页';?>：</th>

        <td><input type='radio' name='info[is_category_index]' value='1'> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;

          <input type='radio' name='info[is_category_index]' value='0' checked> <?php echo L('no');?></td></td>
...
<tr>
  ...
  <th width="200"><?php echo L('available_styles');?>：</th>
	  ...
	  <tr>
        ...
        <th><?php echo '手机站banner';?>：</th>

        <td><?php echo form::images('info[wap_image]', 'wap_image', $wap_image, 'content');?></td>

      </tr> 

      <tr>

     <th><?php echo '是否在手机导航显示';?>：</th>

      <td>

	  <input type='radio' name='info[iswapmenu]' value='1' checked> <?php echo L('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;

	  <input type='radio' name='info[iswapmenu]' value='0'  > <?php echo L('no');?></td>
...
<!--
	$(document).ready(function(){
		$("#template_list option[value='']").removeAttr("selected");
		$("#template_list option[value='default']").attr("selected","selected");
		load_file_list('default');
	...
	function change_tpl(modelid) {
		...
		if(modelid) {
			...
			$.getJSON('?m=admin&c=category&a=public_change_tpl_wap&modelid='+modelid, function(data){$('#template_list').val(data.template_list);$('#wap_category_template').html(data.category_template);$('#wap_list_template').html(data.list_template);$('#wap_show_template').html(data.show_template);});
	...
	function load_file_list(id) {
		...
		$.getJSON('?m=admin&c=category&a=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
			...
			$.getJSON('?m=admin&c=category&a=public_tpl_file_list_wap&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#wap_category_template').html(data.category_template);$('#wap_list_template').html(data.list_template);$('#wap_show_template').html(data.show_template);});
