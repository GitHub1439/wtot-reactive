...
class site extends admin {
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$site_logo = isset($_POST['iimage']) && trim($_POST['iimage']) ? trim($_POST['iimage']) : '';
			$site_taocan = isset($_POST['taocan']) && trim($_POST['taocan']) ? trim($_POST['taocan']) : '0';
			$sy_date = isset($_POST['sy_date']) && trim($_POST['sy_date']) ? trim($_POST['sy_date']) : '';
			$isshiyong = isset($_POST['isshiyong']) && trim($_POST['isshiyong']) ? trim($_POST['isshiyong']) : '0';
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style,'iimage'=>$site_logo,'taocan'=>$site_taocan,'sy_date'=>$sy_date,'isshiyong'=>$isshiyong))) {
	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				$setting = trim(array2string($_POST['setting']));
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$default_style,'iimage'=>$site_logo,'taocan'=>$site_taocan,'sy_date'=>$sy_date,'isshiyong'=>$isshiyong);
