...
class category extends admin {
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			if($_POST['type'] != 2) {
				if (isset($_POST['info']['modelid']) && $_POST['info']['modelid']==0) {
					$_POST['info']['type'] = 1;
					# code...
				}else{
					$_POST['info']['type'] = 0 ;
	...
	public function cache() {
		...
		foreach($this->categorys as $r) {
			...
			$r['trueurl'] = diy_top_link($r['catid']);
			$categorys[$r['catid']] = $r;
		}
	...
	*/
	...
	public function public_tpl_file_list() {
		...
		public function public_tpl_file_list_wap() {
		$style = isset($_GET['style']) && trim($_GET['style']) ? trim($_GET['style']) : exit(0);
		$catid = isset($_GET['catid']) && intval($_GET['catid']) ? intval($_GET['catid']) : 0;
		$batch_str = isset($_GET['batch_str']) ? '['.$catid.']' : '';
		if ($catid) {
		...
		pc_base::load_sys_class('form','',0);
		if($_GET['type']==1) {
			$html = array('page_template'=>form::select_template($style, 'wap',(isset($cat['setting']['wap_page_template']) && !empty($cat['setting']['wap_page_template']) ? $cat['setting']['wap_page_template'] : 'category'),'name="setting'.$batch_str.'[wap_page_template]"','page'));
		} else {
			$html = array('category_template'=> form::select_template($style, 'wap',(isset($cat['setting']['wap_category_template']) && !empty($cat['setting']['wap_category_template']) ? $cat['setting']['wap_category_template'] : 'category'),'name="setting'.$batch_str.'[wap_category_template]"','category'), 
				'list_template'=>form::select_template($style, 'wap',(isset($cat['setting']['wap_list_template']) && !empty($cat['setting']['wap_list_template']) ? $cat['setting']['wap_list_template'] : 'list'),'name="setting'.$batch_str.'[wap_list_template]"','list'),
				'show_template'=>form::select_template($style, 'wap',(isset($cat['setting']['wap_show_template']) && !empty($cat['setting']['wap_show_template']) ? $cat['setting']['wap_show_template'] : 'show'),'name="setting'.$batch_str.'[wap_show_template]"','show')
			);
		}
		if ($_GET['module']) {
			unset($html);
	...
	public function public_change_tpl() {
		...
		public function public_change_tpl_wap() {
		pc_base::load_sys_class('form','',0);
		$models = getcache('model','commons');
		$modelid = intval($_GET['modelid']);
		if($_GET['modelid']) {
			...
			$html = array(
				'template_list'=> $style, 
				'category_template'=> form::select_template($style, 'wap',$category_template,'name="setting[wap_category_template]"','category'), 
				'list_template'=>form::select_template($style, 'wap',$list_template,'name="setting[wap_list_template]"','list'),
				'show_template'=>form::select_template($style, 'wap',$show_template,'name="setting[wap_show_template]"','show')
			);
			if (CHARSET == 'gbk') {
				$html = array_iconv($html, 'gbk', 'utf-8');
			}
