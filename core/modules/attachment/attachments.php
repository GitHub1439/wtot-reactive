...
class attachments {
	 ...
	 * zyFile
	 */	
	public function zyfile() {	 
		$file_path = $this->upload_path.date('Y/md/');
		pc_base::load_sys_func('dir');
		dir_create($file_path);
		$extension = $this->getExt($_FILES['file']['name']);
		$new_file = date('Ymdhis').rand(100, 999).'.'.$extension;
		$filepath = date('Y/md/');
		$uploadfile = $file_path.$new_file;
		if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadfile)) {
				...
				$uploadedfile['filename'] = basename($_FILES['file']['name']); 
				$uploadedfile['fileext']  = strtolower($extension);
				$uploadedfile['filesize'] = $_FILES['file']['size'];
				$uploadedfile['filepath'] = $filepath.$new_file;
				$uploadedfile['module']  = $_POST['module'];
				$uploadedfile['catid']   = $_POST['catid'];
				$uploadedfile['userid']  = $_POST['userid'];
				if (in_array($uploadedfile['fileext'], array('jpg', 'gif', 'jpeg', 'png', 'bmp'))) {
					$uploadedfile['isimage'] = 1;
				}
				$aid = $attachment->add($uploadedfile);
			 $url = pc_base::load_config('system', 'upload_url').$filepath.$new_file;
		     echo trim($url);
		}else {
	...
	public function swfupload(){
		...
		/*	if(empty($this->userid)){
			...
			$file_size = $site_setting['upload_maxsize']*1024;		
