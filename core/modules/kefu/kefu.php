...
class kefu extends admin {
	function __construct() {
		parent::__construct();
		$this->M = new_html_special_chars(getcache('kefu', 'commons'));
		$this->db = pc_base::load_model('kefu_model');
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=kefu&c=kefu&a=add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('link_add'));
	...
	public function edit() {
		if(isset($_POST['dosubmit'])){
			...
			showmessage(L('operation_success'),'?m=kefu&c=kefu&a=edit','', 'edit');
	...
	public function edit_type() {
		if(isset($_POST['dosubmit'])){ 
			...
			showmessage(L('operation_success'),'?m=kefu&c=kefu&a=list_type','', 'edit');
	...
	public function delete() {
  		if((!isset($_GET['linkid']) || empty($_GET['linkid'])) && (!isset($_POST['linkid']) || empty($_POST['linkid']))) {
			...
			if(is_array($_POST['linkid'])){
				...
				showmessage(L('operation_success'),'?m=kefu&c=kefu');
			}else{
				$linkid = intval($_GET['linkid']);
				if($linkid < 1) return false;
				//删除友情链接
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?m=kefu&c=kefu');
				}
			}
			showmessage(L('operation_success'), HTTP_REFERER);
		}
	...
	public function setting() {
		...
		$data = $m_db->select(array('module'=>'kefu'));
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('setting_updates_successful'), '?m=kefu&c=kefu&a=init');
		} else {
			@extract($now_seting);
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=kefu&c=kefu&a=add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('link_add'));
