...
class wap_admin extends admin {
	...
	function type_manage() {
		...
		if($_POST['dosubmit']) {
			...
			foreach($result as $r) {
				...
				$r['select_cat'] = form::select_category_yf('',$r[cat],'name="cat['.$r['typeid'].']"',L('wap_type_bound'),0,0,0,$siteid,0);
				$array[$r['typeid']] = $r;
			}
	...
	public function public_show_cat_ajx() {
		...
		$siteid = intval($_GET['siteid']);
		echo form::select_category_yf('',0,'name="addcat['.$parentid.'][]"',L('wap_type_bound'),0,0,0,$siteid);
	}
	...
	function import_type(){
		...
		$result = $category_db->select(array('siteid'=>$siteid)); 
		foreach ($result as $rs) {
		$info['cat'] = $rs['catid'];
		$info['parentid'] = $rs['parentid'];
		$info['typename'] = $rs['catname'];
		$info['siteid'] = $rs['siteid'];
		$info['listorder'] = $rs['listorder'];
		if($this->type_db->get_one(array('cat'=>$info['cat'],'siteid'=>$siteid))) {
				// $this->type_cache($siteid);
				// showmessage(L('wap_repeat_bound_error'),HTTP_REFERER);
			} else {
				$cat = $this->type_db->get_one(array('cat'=>$info['parentid'],'siteid'=>$siteid));
				if($cat){
					$info['parentid'] = $cat['typeid'];
				}
				$this->type_db->insert($info);		
			}
