...
class index {
	...
	public function lists() {
		...
		$type = wap_cattype($typeid);
		$get_child = wap_get_child($typeid);
		
		if($type!=0){
		$tablename = tablename_by($type);
		//$this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
		$tablename = $this->db->table_name =  $this->db->db_tablepre.$tablename;
