...
class form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		if($toolbar == 'basic') {
			...
			$toolbar = "['Fullscreen', 'Source', '|', 'Bold', 'Italic', 'Forecolor', 'Superscript', 'Subscript', 'Indent', '|', 'Justifyleft', 'Justifycenter', 'Justifyright', 'Justifyjustify','|', 'InsertOrderedList', 'InsertUnorderedList',  'Horizontal', '|', 'Link', 'Unlink', 'SimpleUpload', 'InsertImage', 'Insertvideo', 'Attachment', 'Inserttable', 'Wordimage', 'Insertframe', '|', 'Paragraph', 'Fontfamily', 'Fontsize', '|', 'Removeformat', 'AutoTypeSet','Undo', 'Redo']";
		} else {
			
			$toolbar = "['Fullscreen', 'Source', '|', 'Bold', 'Italic', 'Forecolor', 'Superscript', 'Subscript', 'Indent', '|', 'Justifyleft', 'Justifycenter', 'Justifyright', 'Justifyjustify','|', 'InsertOrderedList', 'InsertUnorderedList',  'Horizontal', '|', 'Link', 'Unlink', 'SimpleUpload', 'InsertImage', 'Insertvideo', 'Attachment', 'Inserttable', 'Wordimage', 'Insertframe', '|', 'Paragraph', 'Fontfamily', 'Fontsize', '|', 'AutoTypeSet', 'Removeformat','Undo', 'Redo']";
   ...
   //          'Print', 'Preview', 'Searchreplace', 'Help']";
		...
		$str .= "var editor = new baidu.editor.ui.Editor({serverUrl:'".JS_PATH."ueditor/php/controller.php?siteid=".get_siteid()."',".join(",",$opt)."});editor.render('$textareaid');\n";
		$str .= '</script>';
	...
	function images_more($field, $value, $fieldinfo) {
		extract($fieldinfo);
		$list_str = '';
		if($value) {
		...
		<fieldset class="blue pad-10" style="width: 600px;">
		...
		$string .= $str."<div class='picBut cu'><a href='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$this->catid','{$authkey}')\"/> ".L('添加图片')." </a></div>";
		return $string;
	}	
