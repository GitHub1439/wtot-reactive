...
function remove_xss($string) { 
    ...
    $parm1 = Array('javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'link', 'script', 'embed', 'object', 'frame', 'frameset', 'ilayer', 'layer', 'bgsound', 'title', 'base');
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array() , $setpages = 10) {
    ...
    if ($num > $perpage) {
        ...
        if ($GLOBALS['static_ly'] == 'wap') {
            //检查USER_AGENT
            if ($curr_page == 1) {
                $multipage.= ' <li class="am-pagination-prev ">
                <a href="javascript:void(0);" class="">' . L('previous') . '</a>
            </li><li class="am-pagination-select">
                <select onchange="parent.location.href=options[selectedIndex].value" >';
            } else {
                $multipage.= ' <li class="am-pagination-prev ">
                <a href="' . pageurl($urlrule, $curr_page - 1, $array) . '" class="">' . L('previous') . '</a>
            </li><li class="am-pagination-select">
                <select  onchange="parent.location.href=options[selectedIndex].value" >';
            }
            for ($i = 1; $i <= $pages; $i++) {
                if ($i != $curr_page) {
                    $multipage.= '<option value="' . pageurl($urlrule, $i, $array) . '">' . $i . ' / ' . $pages . '</option>';
                } else {
                    $multipage.= '<option selected="selected">' . $i . ' / ' . $pages . '</option>';
                }
            }
            if ($curr_page == $pages) {
                $multipage.= '</select>
            </li><li class="am-pagination-next ">
                <a href="javascript:void(0);" class="">' . L('next') . '</a>
            </li>';
            } else {
                $multipage.= '</select>
            </li><li class="am-pagination-next ">
                <a href="' . pageurl($urlrule, $curr_page + 1, $array) . '" class="">' . L('next') . '</a>
            </li>';
            }
        } else {
            if (stristr($_SERVER['HTTP_VIA'], "wap")) {
                ...
                $multipage.= '<div class="box">';
                if ($curr_page > 0) {
                    $multipage.= ' <a href="' . pageurl($urlrule, $curr_page - 1, $array) . '"><font>&lt;</font></a>';
                    if ($curr_page == 1) {
                        $multipage.= ' <a class="on">1</a>';
                ...
                if ($curr_page < $pages) {
                    if ($curr_page < $pages - 5 && $more) {
                        $multipage.= ' <span>...</span> <a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><font>&gt;</font></a>';
                    } else {
                        $multipage.= ' <a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><font>&gt;</font></a>';
                    }
                } elseif ($curr_page == $pages) {
                    $multipage.= ' <a class="on">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page, $array) . '"><font>&gt;</font></a>';
                } else {
                    $multipage.= ' <a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a> <a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><font>&gt;</font></a>';
                }
                $multipage.= '</div>';
...
function pageurl($urlrule, $page, $array = array()) {
	...
	$url = str_replace(array('http://','//','~'), array('~','/','http://'), $url);
	 ...
	 if ($GLOBALS['static_ly'] == 'wap') {
		$wap_html_root = pc_base::load_config('system','wap_html_root');
	 	$url = $wap_html_root.$url;
	 }elseif($GLOBALS['static_ly'] == 'pc'){
...
function catpos($catid, $symbol=' <i>&gt;</i> '){
	$category_arr = array();
	$dqcatid = $catid;
	$siteids = getcache('category_content','commons');
	$siteid = $siteids[$catid];
	$category_arr = getcache('category_content_'.$siteid,'commons');
	if(!isset($category_arr[$catid])) return '';
	$pos = '<a href="'.APP_PATH.'" title="">首页</a>';
	$siteurl = siteurl($category_arr[$catid]['siteid']);
	$arrparentid = array_filter(explode(',', $category_arr[$catid]['arrparentid'].','.$catid));
	foreach($arrparentid as $catid) {
		//$url = $category_arr[$catid]['url'];
		$url = diy_top_link($catid);
		if(strpos($url, '://') === false) $url = $siteurl.$url;
		if($dqcatid == $catid){
		$pos .= $symbol.'<em>'.$category_arr[$catid]['catname'].'</em>';
		}else{
		$pos .= $symbol.'<a href="'.$url.'">'.$category_arr[$catid]['catname'].'</a>';	
		}
	}
	return $pos;
}
...
function subcat($parentid = NULL, $type = NULL,$self = '0', $siteid = '') {
	...
	foreach($category as $id=>$cat) {
		if($cat['siteid'] == $siteid && $cat['ismenu'] == 1 && ($parentid === NULL || $cat['parentid'] == $parentid) && ($type === NULL || $cat['type'] == $type)) $subcat[$id] = $cat;
...
function go($catid,$id, $allurl = 0) {
	...
	if ($id) {
		...
		$r = $db->get_one('', '`id`' , 'listorder DESC');
		$id = intval($r['id']);
	}
